...
void mcsm::GroupStartSubCommand::execute(const std::vector<std::string>& args){
    if(args.empty()){
        ...
        mcsm::warning("Example usage: mcsm group start /path/to/server1 /path/to/server2 random_server_name");
        std::exit(1);
    }
...
void mcsm::GroupStartSubCommand::startAll(const bool& strict){
    ...
    for(auto* config : this->loader->getServers()){
        ...
        mcsm::VoidResult stRes = this->manager->start(cPath);
        if(!stRes){
            if(strict){
                mcsm::error("Failed to start server \"" + cPath + "\".");
    ...
    mcsm::info(startedServers == 0 ? "No servers started." : std::to_string(startedServers) + "/" + std::to_string(this->loader->getServers().size()) + " server(s) started.");
    std::exit(startedServers > 0 ? 0 : 1);
}

void mcsm::GroupStartSubCommand::startWithPaths(const bool& strict, const std::vector<std::string>& serverArgs){
    int startedServers = 0;
    ...
    for(const std::string& path : pathsToStart){
        mcsm::VoidResult stRes = this->manager->start(path);
        if(!stRes){
            if(strict){
